...
export default (): AppConfig => ({
  ...
  mongodbUri:
    ...
    'mongodb://127.0.0.1:27017/research_ustad',
  ...
  workshopDb: {
    ...
    database:
      process.env.WORKSHOP_DB_DATABASE ??
      'research_ustad_workshop',
  },

  workshopId:
    process.env.WORKSHOP_ID ?? 'workshop-one',
  ...
  admin: {
    username:
      process.env.ADMIN_USERNAME ?? 'researchustad',
    email:
      process.env.ADMIN_EMAIL ?? 'admin@researchustad.org',
    seedPassword:
      process.env.ADMIN_SEED_PASSWORD ?? '',
  },

  deviceHashSalt:
    process.env.DEVICE_HASH_SALT ?? '',
  ...
  socket: {
    pingTimeout: parseInt(
      process.env.SOCKET_PING_TIMEOUT ?? '20000',
      10,
    ),
